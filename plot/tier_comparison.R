...
#Load Data
AF <- read.csv("AF_auc_tier.csv")
BC <- read.csv("BC_auc_tier.csv")
CAD <- read.csv("CAD_auc_tier.csv")
DIA <- read.csv("DIA_auc_tier.csv")
LDL <- read.csv("LDL_R2_tier.csv")
HDL <- read.csv("HDL_R2_tier.csv")
TC <- read.csv("TC_R2_tier.csv")
TG <- read.csv("TG_R2_tier.csv")

#Plot
point <- geom_point(position = position_dodge(width = 0.8),size=3)
errorbar_B <- geom_errorbar(aes(ymin = auc_LL, ymax = auc_UL),
    position = position_dodge(width = 0.8), width = 0.1, size = 0.15) 
...
AF_plot <- ggplot(data = AF, aes(x = ethnic, col = tier, shape = tier, y = auc)) + 
 point + errorbar_B + mytheme + y_con_B + lab_name_B + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

BC_plot <- ggplot(data = BC, aes(x = ethnic, col = tier, shape = tier, y = auc)) + 
 point + errorbar_B + mytheme + y_con_B + lab_name_B + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

CAD_plot <- ggplot(data = CAD, aes(x = ethnic, col = tier, shape = tier, y = auc)) + 
 point + errorbar_B + mytheme + y_con_B + lab_name_B + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

DIA_plot <- ggplot(data = DIA, aes(x = ethnic, col = tier, shape = tier, y = auc)) + 
 point + errorbar_B + mytheme + y_con_B + lab_name_B + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

HDL_plot <- ggplot(data = HDL, aes(x = ethnic, col = tier, shape = tier, y = rsquared)) + 
 point + errorbar_Q + mytheme + y_con_Q + lab_name_Q + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

LDL_plot <- ggplot(data = LDL, aes(x = ethnic, col = tier, shape = tier, y = rsquared)) + 
 point + errorbar_Q + mytheme + y_con_Q + lab_name_Q + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

TC_plot <- ggplot(data = TC, aes(x = ethnic, col = tier, shape = tier, y = rsquared)) + 
 point + errorbar_Q + mytheme + y_con_Q + lab_name_Q + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

TG_plot <- ggplot(data = TG, aes(x = ethnic, col = tier, shape = tier, y = rsquared)) + 
 point + errorbar_Q + mytheme + y_con_Q + lab_name_Q + scale_color_jama() +
 scale_shape_manual(values=rep(15, 6)) + guides(color = guide_legend(nrow = 1))

pdf("figure/prediction_accuracy_of_AnnoPred_PRS_based_on_different_tiers.pdf",
...
dev.off()
embedFonts(path.expand("figure/prediction_accuracy_of_AnnoPred_PRS_based_on_different_tiers.pdf"))
